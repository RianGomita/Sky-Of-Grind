...
GTCEuStartupEvents.registry('gtceu:machine', event => {	
    event.create('large_dehydration_unit', 'multiblock')
		...
		.recipeTypes(['dehydration_unit', (GTRecipeTypes.PYROLYSE_RECIPES)])
    ...
    event.create('large_polymerization_chamber', 'multiblock')
        ...
        .pattern(definition => FactoryBlockPattern.start()
			...
			.aisle("WWW##BBBKBBB##AAAAA","WGW##BGGGGGB##AGGGA","WGW##BGGGGGB##AGGGA","WGW##BGGGGGB##AGGGA","WWW##BBBBBBB##AAAAA","################A##","################A##","###################")
                ...
                .where("G", Predicates.blocks("kubejs:peek_reinforced_glass"))
                .where("C", Predicates.blocks("kubejs:resonant_essence_coil_block"))
                .where("W", Predicates.blocks("gtceu:watertight_casing"))
			    .where("A", Predicates.blocks("kubejs:peek_plated_heat_resisted_casing"))
			    .where("S", Predicates.blocks("gtceu:superconducting_coil"))
			    .where("D", Predicates.blocks("gtceu:extreme_engine_intake_casing"))
			    .where("P", Predicates.blocks("kubejs:machine_casing_pipe_peek"))
			    .where("I", Predicates.blocks("gtceu:tungstensteel_pipe_casing"))
			    .where("H", Predicates.blocks("gtceu:heat_vent"))
                .where("M", Predicates.abilities(PartAbility.MUFFLER).setExactLimit(1))
                .where('B', Predicates.blocks('kubejs:machine_casing_inert_peek').setMinGlobalLimited(60) 
                    .or(Predicates.abilities(PartAbility.MAINTENANCE).setExactLimit(1))
                    .or(Predicates.abilities(PartAbility.EXPORT_ITEMS, PartAbility.EXPORT_FLUIDS, PartAbility.INPUT_ENERGY, PartAbility.INPUT_LASER, PartAbility.IMPORT_FLUIDS, PartAbility.IMPORT_ITEMS)))
            .build())
